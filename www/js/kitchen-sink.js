...
/* ===== Modals Page events  ===== */
myApp.onPageInit('status-msisdn', function (page) {
    $$('.demo-alert').on('click', function () {
	myApp.alert('Input data dilakukan oleh TL setelah menerima formulir konsumer');		
    });
	$$('.demo-alert1').on('click', function () {
	myApp.alert('Validasi dilakukan di Back Office dengan durasi H+1 setelah Input Data')
	});
	$$('.demo-alert2').on('click', function () {
    myApp.alert('Pada proses ini, nomor HALO sudah memiliki aktivasinya'); 
	});
...
myApp.onPageInit('search-msisdn', function (page) {
    // Dummy Content
...
$$('.login-screen').find('.button').on('click', function () {
	...
	myApp.showPreloader('Please wait a moment');
        setTimeout(function () {
            myApp.hidePreloader();
        }, 500);
    myApp.alert('Welcome : ' + username, function () {
...
myApp.onPageInit('search-msisdn', function (page) {
    ...
    var virtualList = myApp.virtualList($$(page.container).find('.virtual-list'), {
        ...
        template: '<li>' +
                    '<a href="statusmsisdn.html" class="item-link item-content">' +
                      '<div class="item-inner">' +
                        '<div class="item-title-row">' +
